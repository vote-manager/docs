...
{% hint style="warning" %}
The `platform.next_vote` and `platform.next_vote.hours` tags are only supported in message content, embed field values and embed descriptions due to a limitation of Discord's formatting. Using these tags in other fields will cuase unexpected formatting
{% endhint %}
